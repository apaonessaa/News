# API Endpoint
    ...
    curl -i -X POST --header 'Content-Type: application/json' --data '{"name":"world", "description": "The word is World.", "subcats":[]}' http://localhost:8180/api/categories
    
    curl -i -X GET http://localhost:8180/api/categories/world

    curl -i -X POST --header 'Content-Type: application/json' --data '{"name":"europe", "description": "EU stands for Europe.", "category":"world", "articles": []}' http://localhost:8180/api/categories/world/subcategories
